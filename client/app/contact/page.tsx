    ...
    <>
      ...
      <section className="bg-[#fcfcfc] pt-[calc(72px+3rem)] pb-0 md:pt-[calc(72px+5rem)] relative overflow-hidden">
        ...
        <div className="container-site pb-16 md:pb-20">
          <p className="font-display text-2xl text-primary mb-2">Get in Touch</p>
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Book a Lesson</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            ...
            <div className="flex flex-col gap-6">
              <a href="https://wa.me/94710427241" target="_blank" rel="noopener noreferrer"
                ...
                <p className="text-base font-medium opacity-90 mb-6">The fastest way to book — we reply within minutes.</p>
                <span className="bg-white text-[#25D366] px-8 py-3 text-sm font-black uppercase tracking-widest rounded-full">Message Now</span>
              </a>

              <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4">
                {contactInfo.map(({ icon: Icon, label, lines, href }) => (
                  <div key={label} className="flex gap-4 p-4 bg-white rounded-2xl shadow-sm">
                    <div className="h-10 w-10 bg-primary flex items-center justify-center shrink-0 rounded-xl">
                    ...
                    <div>
                      ...
                      {lines.map((line, i) =>
                        href && i === 0 ? (
                          <a key={i} href={href} className="block text-sm font-medium text-gray-800 hover:text-primary transition-colors">{line}</a>
                        ) : (
              ...
              <a href="https://maps.google.com/?q=Hirikatiya+Beach+Sri+Lanka" target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 border-2 border-gray-800 py-3 rounded-full text-sm font-bold uppercase tracking-wide text-gray-800 hover:border-primary hover:text-primary transition-colors">
                <Navigation className="h-4 w-4" />
                Get Directions
              </a>
            </div>

            <div className="overflow-hidden rounded-2xl shadow-2xl w-full aspect-[4/3] min-h-[340px]">
              <iframe
