...
import { Check, Clock, Users, Gift, ChevronRight, Shield, LifeBuoy } from 'lucide-react'
import { surfPackages, PACKAGE_LEVELS, INCLUDED_IN_ALL, type SurfPackage } from '@/data/packages'
...
export default function PackagesPage() {
  ...
  const filtered = useMemo(
    () => (activeLevel === 'All' ? surfPackages : surfPackages.filter((p) => p.level === activeLevel)),
    [activeLevel],
  )
    ...
    <>
      ...
      <section className="bg-[#1d4ed8]">
        <div className="container-site py-14 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="text-white text-2xl font-bold">Not sure which package suits you?</h2>
            <p className="text-blue-200 text-sm mt-1">
              Our team is happy to recommend the right lesson for your level and goals.
            </p>
          </div>
          <Link
            href="/contact"
            className="shrink-0 inline-flex items-center gap-2 bg-white text-[#1d4ed8] px-8 py-3 text-sm font-bold uppercase tracking-wide hover:bg-blue-50 transition-colors group"
          >
            Get in Touch
