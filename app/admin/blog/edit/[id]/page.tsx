...
import { CATEGORIES, type BlogPost } from '@/data/blogs'
import { cn } from '@/lib/utils'

type FormData = Omit<BlogPost, 'id' | 'readTime' | 'date'>
...
export default function EditBlogPage({
  ...
  const { id } = use(params)
  const { getBlogById, updateBlog, mounted } = useBlogs()
  ...
  if (!mounted) {
    return (
      <div className="pt-[72px] animate-pulse">
  ...
  if (notFound) {
    return (
      <div className="pt-[calc(72px+4rem)] text-center container-site">
        ...
        <Link href="/admin/blog" className="text-[#1d4ed8] underline text-sm">
          Back to admin
        </Link>
      </div>
    )
        ...
        <div className="container-site max-w-4xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-[var(--shadow-card)] p-6 space-y-5">
              ...
              <Field
                label="Full Content"
                hint="## for headings, blank lines for paragraphs"
